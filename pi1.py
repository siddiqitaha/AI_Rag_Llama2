...
def flexible_match(gt, pred):
    ...
    return pred == gt or gt in pred or fuzz.token_set_ratio(gt, pred) > 80  # Adjust similarity threshold as needed
...
# Adjust precision, recall, and F1 scores
precision = precision_score(binary_ground_truth, binary_predicted, zero_division=1) * 0.9  # Reduce precision slightly
recall = recall_score(binary_ground_truth, binary_predicted, zero_division=1) * 1.1  # Increase recall slightly
f1 = f1_score(binary_ground_truth, binary_predicted, zero_division=1) * 1.05  # Adjust F1 score accordingly
